      ...
      <div className="pt-14 px-4 flex items-center justify-between">
        <div>
          ...
          <div className="flex items-center gap-1.5 mt-0.5">
            <motion.div animate={{ scale: [1, 1.3, 1] }} transition={{ duration: 2, repeat: Infinity }} className="w-2 h-2 rounded-sm bg-emerald-400 shrink-0" />
            <span className="text-emerald-400 text-xs">Online · Richco Knowledge Base</span>
          </div>
        </div>
        <button onClick={clearChat} className="w-9 h-9 rounded-lg bg-bg-surface dark:bg-bg-surface-dark flex items-center justify-center border border-slate-200 dark:border-slate-700">
      ...
      <div className="flex gap-2 px-4 mt-4">
        {(['chat', 'docs'] as const).map(t => (
          <button
            ...
            className={`px-5 py-2 rounded-lg text-sm font-semibold capitalize transition-colors ${activeTab === t ? 'bg-green-600 text-slate-900' : 'bg-bg-elevated dark:bg-bg-elevated-dark text-slate-400 dark:text-slate-500 border border-white/10 dark:border-white/5'}`}
          >
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
            {chatMessages.length === 0 && (
              <div className="flex flex-col items-center text-center pt-6 pb-4">
                <div className="w-14 h-14 rounded-xl bg-green-600/15 border border-green-600/20 flex items-center justify-center mb-4">
      ...
      {msg.sources.map((s, i) => {
        ...
        return (
          ...
          >
            ...
            <ChevronRight size={11} className="text-slate-600 dark:text-slate-500 shrink-0 ml-auto" />
          </button>
        )
      })}
    </div>
                    ...
                    {/* Video card */}
                    {msg.videoCard && (
                      <div className="bg-bg-elevated dark:bg-bg-elevated-dark border border-white/10 dark:border-white/5 rounded-xl overflow-hidden flex items-center gap-3 p-3">
          ...
          <div className="px-4 pb-4 pt-2 border-t border-slate-200 dark:border-slate-700 shrink-0">
            <div className="flex items-center gap-2 bg-bg-surface dark:bg-bg-surface-dark rounded-2xl border border-white/10 dark:border-white/5 px-4 py-3">
              <input
                value={input}
              ...
              />
              <button className="w-8 h-8 rounded-lg bg-bg-elevated dark:bg-bg-elevated-dark flex items-center justify-center shrink-0">
                <Mic size={14} className="text-slate-400 dark:text-slate-500" />
              </button>
              <button
                onClick={() => handleSend()}
                disabled={!input.trim()}
                className="w-8 h-8 rounded-lg bg-green-600 disabled:opacity-30 flex items-center justify-center shrink-0 transition-opacity"
              >
